...
class analizador_Sintactico():
    ...
    def analize_obj(self, tokens, gramatica, rules) -> list:
        self.pila.clear()
        self.entrada.clear()
        res = []
        for a in tokens:
        ...
        tokens.append(["$", "$"])
        self.pila.append(terminal("$", "00", "$"))
        p_Text = "".join([f"{a.elemento}{a.estado}" for a in self.pila])
        res.append([p_Text, ''.join(reversed(self.entrada)), "0"])
        self.header = 0
        header = 0
        while True:
            if tokens[self.header][1] in gramatica[0]:
                ...
                if step:
                    if 'd' in step:
                        ...
                        self.header += 1
                        self.entrada.pop()
                        p_Text = "".join([f"{a.elemento}{a.estado[1:]}" for a in self.pila])
                        res.append([p_Text, "".join(reversed(self.entrada)), f"d{step}"])
                    elif 'r' in step:
                        rule = rules[int(step[1:])]
                        if rule[0] == "End":
                            p_Text = "".join([f"{a.elemento}{a.estado[1:]}" for a in self.pila])
                            res.append([p_Text, ''.join(reversed(self.entrada)), "r0 accepted"])
                            res.append(self.pila.pop())
                            break
                        if rule[0] == [a.token for a in self.pila[-rule[1]:]]:
                            elements = []
                            for a in range(rule[1]):
                                elements.insert(0,self.pila.pop())
                            header = int(self.pila[-1].estado[1:])
                            step2 = gramatica[header + 1][gramatica[0].index(rule[2])]
                            if step2:
                                self.pila.append(no_Terminal(rule[2], elements, step, rule[2]))
                                tokens.insert(self.header, ["E", "E"])
                            p_Text = "".join([f"{a.elemento}{a.estado[1:]}" for a in self.pila])
                            res.append([p_Text, "".join(reversed(self.entrada)), step])
                    else:
                        header = int(step)
                        self.header += 1
                else:
...
class mainWindow(QMainWindow):
    ...
    def entered(self):
        ...
        if self.lnEd_In.text() and gram:
            ...
            res = []
            if gram == "Gramatica 1":
                res = a_Sint.analize_obj(tokens, a_Sint.gramatica1, a_Sint.rules1)
            elif gram == "Gramatica 2":
                res = a_Sint.analize_obj(tokens, a_Sint.gramatica2, a_Sint.rules2)
            arbol = res.pop()
            print(arbol)
    ...
    def uiComponents(self):
        ...
        self.tbl_Rul.horizontalScrollBar().close()
        self.tbl_Rul.close()
